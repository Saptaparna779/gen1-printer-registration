# GOAR-3: Re-registering a printer reuses the old Cloud ID instead of generating a new one
...
## Description
Per the GEN 1 business rules: "Re-registration always generates a new Cloud
ID." Support has observed printers that were factory-reset and
re-registered still reporting the SAME Cloud ID as before the reset, which
is causing stale references in downstream billing/subscription systems that
key off Cloud ID.
Steps to Reproduce:
Register a printer with serial SN-1234 -> note the returned cloud_id.
Without deregistering, call register again with the same serial SN-1234
(simulating a factory reset + re-onboard).
Actual: the second response returns the identical cloud_id from step 1.
Expected: the second response returns a NEW, different cloud_id.
Acceptance Criteria:
Every call to register a printer -- first-time or re-registration --
...
Impact: Medium-high -- affects billing/subscription reconciliation.

## Comments
- **Saptaparna Dasgupta:** This is a test comment from the local agentic QA workflow setup.
If you can see this on the ticket, the post_jira_comment.py script works correctly.
- **Saptaparna Dasgupta:** # Validation Report: GOAR-3
